...
export const CounterGroup: React.FC = () => {
  ...
  const {
    ...
    decrementCounter,
    clearError,
  } = useRealTimeCounters(groupId, accessKey);
  // @endsection
  ...
  useEffect(() => {
    const validateAccess = async () => {
      ...
      try {
        ...
        let group;
        try {
          group = await apiFetch(`/api/groups?name=${groupName}`);
        } catch (err) {
          // Group doesn't exist
        }
  ...
  const handleUpdateGroup = async (updates: {
    ...
    try {
      ...
      if (updates.display_name) {
        setGroupDisplayName(result.display_name);
      }
      if (updates.profile_image_url !== undefined) {
        setGroupProfileImageUrl(result.profile_image_url);
      }
    } catch (err) {
  ...
  const copyShareUrl = async () => {
    ...
    try {
      ...
      setCopyError("Failed to copy URL to clipboard. Please copy manually.");
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center transition-colors duration-200">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Loading group...</p>
        </div>
      </div>
    );
  }
                ...
                {counters.map((counter) => (
                  <CounterCard
                    ...
                    onUpdate={async (updates) => {
                      try {
                        await updateCounter(counter.id, updates);
                      } catch (err) {
