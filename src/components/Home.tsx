...
import { apiFetch } from "../hooks/useSupabase";
...
export const Home: React.FC = () => {
  ...
  useEffect(() => {
    const checkGroupExists = async () => {
      ...
      try {
        ...
        const data = await apiFetch(`/api/groups?name=${cleanGroupName}`);
  ...
  const handleJoinGroup = async (e: React.FormEvent) => {
    ...
    try {
      let group;
      try {
        group = await apiFetch(`/api/groups?name=${cleanGroupName}`);
      } catch (err) {
        // Group doesn't exist, which is fine for creation
      }
      ...
      setFormError("Error validating group access: " + (err as Error).message);
    }
  };
  /* @endsection */
        ...
        <div className="text-center mb-8">
          <div className="bg-gray-100 dark:bg-gray-900 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4">
            <img
              ...
              alt={groupExists ? "Group profile" : "VibeCount logo"}
            />
          </div>
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Create or join a group with counters for anything imaginable! Secure
            and in real-time!
          </p>
        </div>
        ...
        {groupExists && (
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <div className="flex items-center text-blue-600 dark:text-blue-400 text-sm">
              ...
              <span>
                This group already exists. Enter the correct access key to join.
              </span>
            </div>
          </div>
        )}
